...
const tabConfigs: Record<UserType, { key: TabKey; label: string; icon: keyof typeof Ionicons.glyphMap }[]> = {
  gainer: [
...
const Navigation = ({ userType: propUserType, initialTab, onTabChange }: NavigationProps) => {
    ...
    const loadUserData = async () => {
      try {
        ...
        if (userData) {
          ...
          const role = (parsed.role || parsed.userType || parsed.type || "donor").toLowerCase()
  ...
  const setupNotifications = (role: string) => {
    ...
    if (role === "gainer") {
      ...
      socket.on("emergencyAccepted", (data: any) => {
        ...
        setActiveEmergencyRequest((prev: any) => normalizeEmergencyData({
          ...prev,
          ...data,
          status: 'Accepted'
        }))
  ...
  const startSOSAnimation = () => {
    Animated.loop(
      Animated.sequence([
        Animated.parallel([
          Animated.timing(pulseAnim, { toValue: 2, duration: 2000, useNativeDriver: true }),
  ...
  const fetchActiveEmergency = async (userId: string | number, role?: string) => {
    try {
      ...
      if (response.ok && data.requests) {
        const active = data.requests.find((r: any) => r.Status === "Emergency" || r.Status === "Accepted")
        if (active) {
  ...
  const handleStopEmergency = async () => {
    ...
    try {
      const token = await AsyncStorage.getItem("authToken")
      if (!token) return
      const response = await fetch(API_ENDPOINTS.STOP_EMERGENCY(activeEmergencyRequest.requestId || activeEmergencyRequest.RequestId), {
        method: "PATCH",
        headers: { Authorization: `Bearer ${token}` },
      })
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              if (status !== 'granted') {
                Alert.alert('Permission Denied', 'Location permission is required to accept emergency requests.');
                return;
              }
              ...
              const response = await fetch(API_ENDPOINTS.ACCEPT_EMERGENCY(requestId), {
                method: "PATCH",
                headers: {
                  "Content-Type": "application/json",
                  Authorization: `Bearer ${token}`
              ...
              if (response.ok) {
                Alert.alert("Accepted", "The gainer has been notified with your live location.")
                // Keep it in incomingEmergency so they can see the status and "Cancel Help"
                setIncomingEmergency((prev: any) => normalizeEmergencyData({
                  ...prev,
                  status: 'Accepted'
                }))
    ...
    <View style={styles.container}>
      <View style={styles.navBar}>
        {tabs.map((tab) => {
          ...
          return (
            <TouchableOpacity key={tab.key} style={styles.tab} activeOpacity={0.9} onPress={() => handlePress(tab.key)}>
              {isActive ? (
          ...
          onPress={() => {
            if (userType === 'gainer') {
              ...
              if (incomingEmergency) {
                ...
                Alert.alert("No Emergency", "There are no incoming emergency requests at the moment.")
              }
            }
          }}
        />
