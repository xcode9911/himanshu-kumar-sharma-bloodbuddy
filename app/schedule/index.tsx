...
const DONATION_DAY_REMINDER_KEYS = "donationDayReminderKeys";

export default function ScheduleScreen() {
  const { pushInAppNotification } = useNotifications();
  ...
  const triggerDonationDayReminders = async (items: any[]) => {
    try {
      const now = new Date();
      const raw = await AsyncStorage.getItem(DONATION_DAY_REMINDER_KEYS);
      const seen = new Set<string>(raw ? JSON.parse(raw) : []);
      let changed = false;

      items.forEach((item) => {
        if (item.status?.toLowerCase() !== "accepted" || !item.donationDate) {
          return;
        }

        const donationDate = new Date(item.donationDate);
        if (!isSameCalendarDay(donationDate, now)) {
          return;
        }

        const reminderKey = `donation-day-${item.id}-${donationDate.toDateString()}`;
        ...
        const orgName = item.organizationName || item.name || "Blood bank";
        pushInAppNotification({
          title: "Donation Day Reminder",
          message: `${orgName}: your donation is scheduled today at ${donationDate.toLocaleTimeString(
            [],
            {
              hour: "2-digit",
              minute: "2-digit",
          ...
          sourceEvent: "donation-day-reminder",
          showAlert: true,
        });
      });
      ...
      if (changed) {
        await AsyncStorage.setItem(
          DONATION_DAY_REMINDER_KEYS,
          JSON.stringify(Array.from(seen)),
        );
      }
    } catch (error) {
      console.log("Error triggering donation-day reminders:", error);
    }
  };
  ...
  const fetchDonations = async () => {
    try {
      ...
      if (response.ok && data.offers) {
        const mappedDonations = data.offers.map((o: any) => ({
          id: o.OfferId,
          name:
            role === "organization"
              ? o.donor?.user?.FullName || "Donor"
          ...
          phone:
            role === "organization"
              ? o.donor?.user?.Phone || "N/A"
              : o.organization?.Contact || o.organization?.Phone || "N/A",
        ...
        if (role === "donor") {
          triggerDonationDayReminders(mappedDonations);
        }
      }
    } catch (error) {
  ...
  const confirmAcceptance = async () => {
    ...
    if (visitDate < new Date()) {
      Alert.alert(
        "Error",
        "You cannot schedule a visit for a past date or time.",
      );
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        API_ENDPOINTS.UPDATE_DONATION_STATUS(selectedDonation.id),
        {
          method: "PATCH",
          headers: {
      ...
      if (response.ok) {
        Alert.alert(
          "Success",
          "Donation scheduled successfully. Donor will be notified.",
        );
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              const token = await AsyncStorage.getItem("authToken");
              const response = await fetch(
                API_ENDPOINTS.UPDATE_DONATION_STATUS(id),
                {
                  method: "PATCH",
                  headers: {
                    Authorization: `Bearer ${token}`,
        ...
        <View style={{ flexDirection: "row", alignItems: "center" }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.subtitle}>
              Your confirmed and pending visits
            </Text>
          </View>
        </View>
      </View>
        ...
        <View style={styles.tabContainer}>
          ...
          >
            <Text
              style={[
                styles.tabText,
                orgTab === "requests" && styles.activeTabText,
              ]}
            >
              Requests
            </Text>
          ...
          >
            <Text
              style={[
                styles.tabText,
                orgTab === "confirmed" && styles.activeTabText,
              ]}
            >
              Confirmed
            </Text>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalSubtitle}>
              Pick a date and time for the donor to visit your organization.
            </Text>
            ...
            <View style={styles.modalActions}>
              ...
              >
                {accepting ? (
                  ...
                  <Text style={styles.confirmModalText}>Confirm Visit</Text>
                )}
              </TouchableOpacity>
            </View>
          </View>
