...
export default function CampaignQRScreen() {
  ...
  const { id, title, location, orgName, organizationNames } =
    useLocalSearchParams();
  const qrRef = useRef<any>(null);

  const parseOrganizationNames = (raw: unknown): string[] => {
    if (!raw) {
      return [];
    }
  ...
  const allOrganizationNames = Array.from(
    new Set(
      [
        ...parseOrganizationNames(organizationNames),
        String(orgName || "").trim(),
      ].filter(Boolean),
    ),
  );
  ...
  const qrValue = JSON.stringify({
    ...
    campaignId: id,
    title: title,
    organizations: allOrganizationNames,
  });
    ...
    try {
      const html = `
                ...
                <body>
                    ...
                    <div style="font-size: 20px; color: #D11B31; font-weight: bold; margin-bottom: 5px;">${organizationHeading}</div>
                    <div class="title">${title}</div>
                    <div class="location">${location}</div>
                    ${allOrganizationNames.length > 1 ? '<div style="font-size: 14px; color: #6B7280; margin-top: 5px;">Collaborative Campaign</div>' : ""}
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.qrCard}>
          ...
          <View style={styles.orgBadge}>
            <Text style={styles.orgNameText}>{organizationHeading}</Text>
          </View>
          {allOrganizationNames.length > 1 && (
            <Text style={styles.collabTag}>Collaborative Campaign</Text>
          )}
          ...
          <Text style={styles.instruction}>
            Scan to record attendance & auto-update inventory
          </Text>
        </View>
