...
export default function OrganizationDetailScreen() {
    ...
    const bloodTypes = params.bloodTypes ? JSON.parse(params.bloodTypes as string) : []
    ...
    const submitBooking = async () => {
        ...
        try {
            ...
            if (!token) {
                Alert.alert("Error", `Please login to ${userRole === "donor" ? "donate" : "book"} units`)
                return
            }

            const isDonation = userRole === "donor"
            const endpoint = isDonation ? API_ENDPOINTS.CREATE_DONATION : API_ENDPOINTS.CREATE_BOOKING
            ...
            if (response.ok) {
                Alert.alert("Success", isDonation ? "Donation offer submitted successfully!" : "Booking request submitted successfully!")
                setBookingModalVisible(false)
            } else {
                Alert.alert("Error", data?.message || `Failed to submit ${isDonation ? "donation offer" : "booking request"}`)
            }
        } catch (error) {
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
                ...
                {userRole === "donor" && (
                    <TouchableOpacity
                        style={[styles.mainActionButton, styles.donateButton, { marginHorizontal: scale(4) }]}
                        onPress={() => handleActionPress(donorBloodType)}
                    >
                        <DonationSvg width={28} height={28} fill="#FFFFFF" />
                        <Text style={styles.mainActionButtonText}>Donate Now ({donorBloodType})</Text>
                    </TouchableOpacity>
                )}
                ...
                <View style={styles.section}>
                    ...
                    {bloodTypes.length > 0 ? (
                        ...
                        <Text style={styles.emptyText}>No blood inventory available currently.</Text>
                    )}
                </View>
            </ScrollView>
            ...
            >
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <Text style={styles.modalSubtitle}>Blood Type: {selectedBloodType}</Text>
                        ...
                        <View style={styles.modalButtons}>
                            ...
                            <TouchableOpacity
                                style={[styles.modalButton, styles.confirmButton, userRole === "donor" && styles.donateButton]}
                                onPress={submitBooking}
                                disabled={loading}
                            >
                                <Text style={styles.confirmButtonText}>
                                    {loading ? "Submitting..." : (userRole === "donor" ? "Confirm Donation" : "Confirm Booking")}
                                </Text>
                            </TouchableOpacity>
                        </View>
                    </View>
