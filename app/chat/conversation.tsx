...
import { connectSocket, emitPrivateMessage, getSocket } from "../../config/socket";
...
export default function ConversationScreen() {
    const { contactId, contactName, contactRole, organizationName } = useLocalSearchParams();
    ...
    const fetchMessages = async () => {
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/api/chat/messages/${contactId}`, {
                headers: {
                    ...
                    <View style={styles.messageFooter}>
                        ...
                        {isMine && (
                            <Ionicons
                                ...
                                size={16}
                                color={item.status === 'read' ? "#3B82F6" : "rgba(255, 255, 255, 0.6)"}
                    ...
                    <View style={styles.header}>
                        ...
                        <View style={styles.headerTitleContainer}>
                            <Text style={styles.headerTitle} numberOfLines={1}>
                                {organizationName ? (organizationName as string) : (contactName as string)}{" "}
                                <Text style={styles.headerRole}>({contactRole})</Text>
                            </Text>
                            <View style={styles.statusRow}>
                                <View style={[styles.onlineDot, { backgroundColor: isPartnerOnline ? "#10B981" : "#9CA3AF" }]} />
                                <Text style={[styles.headerSubtitleText, !isPartnerOnline && styles.offlineSubtitle]}>
                                    {organizationName ? `${contactName} • ` : ""}{isPartnerOnline ? "Active now" : "Offline"}
                                </Text>
                            </View>
                        </View>
                    </View>
...
const styles = StyleSheet.create({
    ...
    footer: {
        ...
        paddingBottom: Platform.OS === 'ios' ? verticalScale(30) : verticalScale(15),
        borderTopWidth: 1,
    ...
    viewProfileOverlay: {
        ...
        left: 0,
        right: 0,
        backgroundColor: 'rgba(209, 27, 49, 0.7)',
        paddingVertical: 1,
        justifyContent: 'center',
        alignItems: 'center',
